                        ...
                        <table class="table" id="weblinkList">
                            ...
                            <tbody <?php if ($saveOrder) :
                                ...
                                <?php foreach ($this->items as $i => $item) :
                                    ?>
                                    <?php
                                    $item->cat_link = Route::_('index.php?option=com_categories&extension=com_weblinks&task=edit&type=other&cid[]=' . $item->catid);
...
// Check at component level
                        ...
                        <?php if ($user->authorise('core.create', 'com_weblinks') && $user->authorise('core.edit', 'com_weblinks') && $user->authorise('core.edit.state', 'com_weblinks')) :
                            ?>
    ...
    Joomla.submitbutton = function(task) {
        if (task === 'weblinks.delete' && !confirm(Joomla.Text._('JGLOBAL_CONFIRM_DELETE'))) {
            return false;
        }
        if (document.adminForm.boxchecked.value == 0 && (task === 'weblinks.publish' || task === 'weblinks.unpublish' || task === 'weblinks.archive' || task === 'weblinks.trash' || task === 'weblinks.delete' || task === 'weblinks.checkin')) {
            alert(Joomla.Text._('COM_WEBLINKS_NO_ITEM_SELECTED'));
            return false;
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const toolbarButtons = document.querySelectorAll('#toolbar joomla-toolbar-button');
        toolbarButtons.forEach(toolbarButton => {
            const task = toolbarButton.getAttribute('task');
            if (!task) {
                return;
            }

            toolbarButton.addEventListener('click', function(e) {
                const innerButton = toolbarButton.querySelector('button');
                if (innerButton && innerButton.hasAttribute('disabled')) {
                    return;
                }
                e.preventDefault();

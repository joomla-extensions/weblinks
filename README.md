...
#### 4. These files should be in your weblinks folder.
...
You need to install(pre-requisites):
...
Note: It’s better to use the **‘Git Bash’** program for running command-line commands.

This step is only required if you **don't** place the weblinks folder into your web server folder. Create a symbolic link from your tests\joomla-cms to a subfolder of your web server. For example:
...
Once the prerequisites are installed, follow the [**For Linux / MacOS**](#for-linux--macos) section starting from the **Install** step. The commands such as:

- `git clone`  
- `composer install`  
- `npm ci`  

work the same in **Git Bash** on Windows. The only adjustment required is to replace the file paths:  

- For **Linux/MacOS:** `/var/www/html/weblinks`  
- For **Windows (WAMP):** `C:\wamp\www\weblinks`  
- For **Windows (XAMPP):** `C:\xampp\htdocs\weblinks`  

This ensures smooth compatibility across different environments. ✅

## Development and Package Building

Once you have completed setting up the extension in your local environment, you can view your changes directly in the **Administrator Panel** for testing.  

To achieve this, you can use:  
 

 **JROBO for Instant Changes:**  
   - **JROBO** automates the process of building and verifying changes instantly.  
   - It allows you to see your modifications without the need for manually rebuilding the package every time.  
   - Run JROBO to generate and map the symlink to the Joomla installation directory, making the extension available for testing immediately.

✅ To use JROBO effectively:
- Navigate to your extension directory.
- Run these JROBO command:
...
```
The above command let you use the JoRobo.
...
This command Builds your extension into an installable Joomla! package or zip file including replacements, outputting a .zip archive to the dist/ directory.

...
Cypress.Commands.add('api_post', (path, body) => cy.api_getBearerToken().then((token) => cy.request({
  method: 'POST', url: `/api/index.php/v1${path}`, body, headers: { Authorization: `Bearer ${token}` }, json: true, timeout: 60000,
})));

Cypress.Commands.add('api_patch', (path, body) => cy.api_getBearerToken().then((token) => cy.request({
  method: 'PATCH', url: `/api/index.php/v1${path}`, body, headers: { Authorization: `Bearer ${token}` }, json: true, timeout: 60000,
})));
